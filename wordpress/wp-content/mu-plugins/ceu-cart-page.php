...
/**
 * Resolve the header layout "the rest of the site" uses.
 *
 * Mirrors zilom_get_header_layout() but against the FRONT PAGE rather than the
 * current one, so whatever the home page renders is what the cart gets. Reading
 * the site option alone was not enough: an Elementor-built header is normally
 * selected per page through the zilom_page_header meta, and the site-wide option
 * can sit empty while every page still carries the same override.
 *
 * Returns '' when nothing can be determined, so the caller leaves the theme's own
 * answer alone rather than forcing a header that does not exist.
 */
function ceu_site_header_layout(): string {
    ...
    // Only useful if the header actually exists as a gva_header post; otherwise
    // header.php falls through to header-default.php anyway, which is the very
    // template that has no top bar.
...
/**
 * Put the cart and checkout on that same header.
 *
 ...
 * that meta is absent or '__default_option_theme'. The cart page resolves to
 * something else, so it renders header-default.php — which has no blue top bar at
 * all, just the logo row — while the rest of the site gets the Elementor header
 * carrying the email, Sign in / Register and social links.
 *
 ...
 *
 * Same gate style as the debug block in ceu-auth.php. Prints only for a logged-in
 * administrator, so the query string alone reveals nothing to a visitor.
 */
add_action('wp_footer', function () {
    ...
    $front_id = (int) get_option('page_on_front');
    $row = function ($k, $v) {
        return '  ' . str_pad($k, 34) . (is_scalar($v) ? (string) $v : gettype($v)) . "\n";
    };
    ...
    echo $row('is cart page', (function_exists('ceu_is_cart_page') && ceu_is_cart_page()) ? 'YES' : 'no');
    echo $row('is checkout page', (function_exists('ceu_is_checkout_page') && ceu_is_checkout_page()) ? 'YES' : 'no');
    echo $row('this page id', $post_id ?: '(none)');
    echo $row('  its zilom_page_header', $post_id ? (get_post_meta($post_id, 'zilom_page_header', true) ?: '(empty)') : '-');
    echo $row('front page id', $front_id ?: '(none — posts page)');
    ...
    if (!$headers) {
        echo "    (none — every page falls back to header-default.php)\n";
    } else {
        foreach ($headers as $h) echo "    - {$h->post_name}  ({$h->post_status})\n";
    }
    echo '</pre>';
}, 999);
