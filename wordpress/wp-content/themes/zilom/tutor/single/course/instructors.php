...
if ($instructors){
	...
	<div class="course-single-instructors">
		<h4 class="tutor-segment-title">
			<?php 
				if($count > 1){
					echo esc_html__('About the instructors', 'zilom');
				}else{
					echo esc_html__('About the instructor', 'zilom');
				}
			?>	
		</h4>
				...
				<div class="single-instructor-wrap">
					<div class="single-instructor-top">
							...
							<div class="instructor-name">
								...
								<?php
									if ( ! empty($instructor->tutor_profile_job_title)){
										echo '<h4>';
											echo esc_html($instructor->tutor_profile_job_title);
										echo '</h4>';	
									}
								?>
							</div>
						...
						<div class="instructor-bio">
							<?php echo wp_kses($instructor->tutor_profile_bio, true); ?>
						</div>
					</div>
					...
					<div class="single-instructor-bottom">
						<div class="ratings">
							...
							<?php
								...
								echo " <span class='rating-total-meta'>({$instructor_rating->rating_count} ".__('ratings', 'zilom').")</span> ";
							?>
						</div>
						...
						<div class="courses">
							<p>
								...
								<?php echo tutor_utils()->get_course_count_by_instructor($instructor->ID); ?> <span class="tutor-text-mute"> <?php echo esc_html__('Courses', 'zilom'); ?></span>
							</p>
						</div>

						<div class="students">
							<?php $total_students = tutor_utils()->get_total_students_by_instructor($instructor->ID); ?>
							<p>
								<i class='tutor-icon-user'></i>
								<?php echo esc_html($total_students); ?>
								<span class="tutor-text-mute"> <?php echo esc_html__('students', 'zilom'); ?></span>
							</p>
						</div>
					</div>
				</div>
