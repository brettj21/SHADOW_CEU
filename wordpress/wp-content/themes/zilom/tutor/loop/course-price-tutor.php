...
$enroll_btn = '<div class="tutor-course-list-btn">' . apply_filters( 'tutor_course_restrict_new_entry', '<a href="' . get_the_permalink() . '" class="tutor-btn tutor-btn-outline-primary tutor-btn-md tutor-btn-block ' . $required_loggedin_class . '">' . __( 'Enroll Course', 'zilom' ) . '</a>', $course_id ) . '</div>';
		...
		if ( 0 === $maximum_students ) {
			...
			<div class="<?php echo esc_attr( $utility_classes ); ?>">
				...
				<div class="tutor-course-booking-progress tutor-d-flex tutor-align-center">
					...
					<?php echo esc_html( $b_total ) . __( '% Booked', 'zilom' ); ?>
					</div>
				</div>
			</div>
			<?php
